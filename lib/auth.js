...
async function sendOtpViaWhatsApp(phone, code) {
  ...
  if (!token || !phoneId) {
    console.warn('[WhatsApp OTP] WHATSAPP_API_TOKEN / WHATSAPP_PHONE_NUMBER_ID not set — OTP logged only (dev mode).');
    return false;
  }

  // Ensure E.164 format without leading '+' for Meta API
  const to = phone.replace(/^\+/, '').replace(/^91/, '').replace(/^\+?/, '91');
  ...
  const body = {
    ...
    type: 'template',
    template: {
      name: process.env.WHATSAPP_OTP_TEMPLATE || 'cheap_travels_otp',
      language: { code: 'en' },
      components: [{
        type: 'body',
...
// Send OTP — saves to DB, delivers via WhatsApp
async function sendOtp(mobile) {
  ...
  // Always log for dev/debugging visibility
  console.log(`[OTP] ${mobile} → ${code} (expires in 5 min)`);
