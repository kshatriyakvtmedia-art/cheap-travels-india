...
function SelectSeatsContent() {
  ...
  useEffect(() => {
    fetch(`/api/buses?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}&date=${date}`)
      .then(r => r.json())
      .then(d => {
        const b = d.buses?.find(x => x.externalId === decodeURIComponent(busId));
        setBus(b || null);
        if (b) {
  ...
  if (!bus) {
    return (
      <div className="max-w-7xl mx-auto p-8 text-gray-500 flex flex-col items-center gap-3">
    ...
    <div className="bg-brand-surface min-h-screen pb-12">
      <div className="bg-brand-green text-white py-4">
        <div className="max-w-7xl mx-auto px-4 flex flex-wrap items-center gap-3">
          <Link
            href={`/results?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}&date=${date}`}
            className="text-emerald-100 text-sm hover:text-white"
        ...
        <div className="card p-5">
          <div className="flex gap-5 border-b border-gray-200 mb-5 text-sm overflow-x-auto">
            {[['seats', 'Seats'], ['boarding', 'Boarding'], ['dropping', 'Dropping'], ['policy', 'Policy']].map(([k, l]) => (
              <button key={k} onClick={() => setTab(k)}
                className={`pb-2 border-b-2 whitespace-nowrap ${tab === k ? 'border-brand-green text-brand-green font-bold' : 'border-transparent text-gray-500'}`}>{l}</button>
            ))}
          </div>
          ...
          {tab === 'boarding' && (
            <div className="space-y-2">
              {bus.boardingPoints?.map(p => <PointCard key={p.id} p={p} />) || <p className="text-sm text-gray-500">No boarding points available.</p>}
            </div>
          )}
          {tab === 'dropping' && (
            <div className="space-y-2">
              {bus.droppingPoints?.map(p => <PointCard key={p.id} p={p} />) || <p className="text-sm text-gray-500">No dropping points available.</p>}
            </div>
          )}
          {tab === 'policy' && (
            <div className="text-sm text-gray-600 space-y-2 max-w-lg">
              <p><b>Cancellation:</b> Cancellation charges vary by operator. Generally, cancellations made 24+ hours before departure are refundable minus the operator fee.</p>
...
export default function SelectSeatsPage() {
  return (
    <Suspense fallback={<div className="max-w-7xl mx-auto p-8 text-gray-500">Loading seat map...</div>}>
      <SelectSeatsContent />
    </Suspense>
  );
}
